...
export default function Teams() {
  ...
  // Keep all the existing useEffect and handler functions exactly as they are
  useEffect(() => {
    const fetchUsersAndConnections = async () => {
      if (!user) return;
  ...
  // Keep all the original handler functions
  ...
  // Keep all other original functions exactly as they are
  ...
  const updateUserConnections = (userId, increment) => {
    // ... Keep the original implementation
  };

  const getRequestStatus = (targetUser) => {
    const existingRequest = requests.find(
      req => 
        (req.senderId === user?.uid && req.receiverId === targetUser.uid) ||
        (req.senderId === targetUser.uid && req.receiverId === user?.uid)
        ...
        <div className="bg-zinc-800 rounded-lg p-6 mb-8">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                className="w-full pl-10 pr-4 py-2 rounded-md bg-zinc-850 border-zinc-700 text-black  placeholder-gray-400"
                placeholder="Search by name or skill..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
          ...
          <div className="mb-4">
            <h3 className="text-sm font-medium text-gray-300 mb-2">Experience Level</h3>
            <div className="flex gap-2 flex-wrap">
              {['beginner', 'intermediate', 'advanced'].map(level => (
                <button
                  key={level}
                  ...
                  className={`px-3 py-1 rounded-full text-sm ${
                    selectedExperience.includes(level)
                      ? "relative overflow-hidden px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-purple-500 to-pink-500 shadow-lg transition-all duration-300 ease-in-out hover:from-pink-500 hover:to-purple-500 before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-white/10 before:scale-0 hover:before:scale-100 before:transition-transform before:duration-300 before:rounded-lg"
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <h3 className="font-semibold text-white">{targetUser.displayName}</h3>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {targetUser.skills?.map((skill, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center px-2 py-1 rounded-md bg-zinc-800 text-gray-300 text-xs"
                    >
                      {skillCategories.find(cat => cat.id === skill.category)?.icon}
                      <span className="ml-1">{skill.name}</span>
                    </span>
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-gray-400 capitalize">{selectedUser.experience} Developer</p>
                      <p className="text-sm text-gray-500">
                        {selectedUser.connections} Connections
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedUser.skills?.map((skill, index) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-3 py-1 rounded-full bg-zinc-800 text-gray-300"
                        >
                          {skillCategories.find(cat => cat.id === skill.category)?.icon}
                          <span className="ml-2">{skill.name}</span>
                          <span className="ml-1 text-gray-500">• {skill.level}</span>
